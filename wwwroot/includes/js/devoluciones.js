...
$(document).ready(function () {
    ...
    $("#btnProcesarPrecarga").click(function () {
        ...
        if (ItemsPrecargados.length > 0) {
            ...
            var json = JSON.stringify(ItemsPrecargados);
            $.ajax({
                type: "POST",
    ...
    $("#btnProcesarPrecargaVencidos").click(function () {
        ...
        if (ItemsPrecargadosVencidos.length > 0) {
            ...
            showCargandoBuscador();
            var json = JSON.stringify(ItemsPrecargadosVencidos);
            $.ajax({
                type: "POST",
                url: "/devoluciones/AgregarSolicitudDevolucionCliente",
                data: json,
    ...
    $("#btnProcesarPrecargaFNE").click(function () {
        ...
        if (ItemsPrecargadosFNE.length > 0) {
            ...
            var json = JSON.stringify(ItemsPrecargadosFNE);
            $.ajax({
                type: "POST",
    ...
    $("#btnProcesarPrecargaFC").click(function () {
        ...
        ControlarSesion();
        var json = JSON.stringify(ItemsPrecargadosFC);
        $.ajax({
            type: "POST",
            url: "/devoluciones/AgregarSolicitudDevolucionCliente",
            data: json,
            ...
            if (ItemsDev.length > 0) {
                ...
                for (i = 0; i < ItemsDev.length; i++) {
                    if (ItemsDev[i].dev_cantidad > 0) {
                        ...
                        html += "<td>" + PRD + "</td>";
                        html += "<td>" + colMotivos[parseInt((ItemsDev[i].dev_motivo) + 0)] + "</td>";
...
function Imprimir() {
    ...
    if (colMotivos[parseInt((ItemsDev[0].dev_motivo) + 0)] == 'Producto Vencido') {
        esDevVencidos = true;
    }
    ...
    for (var hoja = 1; hoja <= cantHojas; hoja++) {
        ...
        for (i = (hoja - 1) * 20; i < ((hoja * 22) - 1); i++) {
            if (i < ItemsDev.length) {
                ...
                if (!esDevVencidos) {
                    html += "<td style=' font-size:9px !important;'>" + colMotivos[parseInt((ItemsDev[i].dev_motivo) + 0)] + "</td>";
                }
