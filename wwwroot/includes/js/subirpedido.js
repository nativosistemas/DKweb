...
jQuery(document).ready(function () {
  if (historialPrimerRegistro == null) {
    historialPrimerRegistro = eval(
      "(" + $("#hiddenPrimerArchivoSubidos").val() + ")"
    );
  ...
  var aux_ArchivoSubirListaProductos = $(
    "#hiddenArchivoSubirListaProductos"
  ).val(); //eval('(' + $('#hiddenArchivoSubirListaProductos').val() + ')');
...
function subirArchivoMsgRepetido() {
  ...
  strHtml +=
    '<p class="text-center">Este archivo ya ha sido cargado ¿Desea cargarlo nuevamente?</p>';
  strHtml += "<p>&nbsp;</p>";
  strHtml += "<p>&nbsp;</p>";
  strHtml += '<div class="clear10"></div>';
  strHtml +=
    '<a class="btn_confirmar" href="subirarchivoresultado">ACEPTAR</a>';
  strHtml +=
    '<a class="btn_vaciar float-left" href="#" data-dismiss="modal">CANCELAR</a>';
  strHtml += "</div>";
...
function CargarDatoPrimerHistorial() {
  if (historialPrimerRegistro != null) {
    ...
    strHtmlUltimoPedido +=
      '<div class="est_ped st">&Uacute;ltimo pedido realizado:<a class="p_lr" href="#" data-toggle="modal" data-target="#modalSubirarchivo"  onclick="onclickCargar(' +
      historialPrimerRegistro.has_id +
      ');return false;">';
    strHtmlUltimoPedido +=
      historialPrimerRegistro.has_NombreArchivoOriginal + "</a>";
    var strNombreSucursal0 = "";
    if (historialPrimerRegistro.suc_nombre != null) {
      strNombreSucursal0 = historialPrimerRegistro.suc_nombre;
    }
    strHtmlUltimoPedido +=
      '<div class="clear10 visible-xs"></div>' + strNombreSucursal0 + " - ";
...
function CargarUnidadesRenglones() {
  if (listaProductosBuscados != null && listaSucursal != null) {
    ...
    for (
      ...
      for (
        var iSucursalStocks = 0;
        iSucursalStocks <
        listaProductosBuscados[iProducto].listaSucursalStocks.length;
        iSucursalStocks++
      ) {
        ///
        var indexSucursal_temp = 0;
        for (
          var iEncabezadoSucursal_aux = 0;
          iEncabezadoSucursal_aux < listaSucursal.length;
          iEncabezadoSucursal_aux++
        ) {
          if (
            listaProductosBuscados[iProducto].listaSucursalStocks[
              iSucursalStocks
            ].stk_codsuc === listaSucursal[iEncabezadoSucursal_aux]
          ) {
        ...
        if (isNotNullEmpty(nroValor)) {
          ...
          if (nroValor > 0) {
            for (
              var iSucursalesUnidadesRenglones = 0;
              iSucursalesUnidadesRenglones <
              listaSucursalesUnidadesRenglones.length;
              iSucursalesUnidadesRenglones++
            ) {
              if (
                listaSucursalesUnidadesRenglones[iSucursalesUnidadesRenglones]
                  .codSucursal ==
                  listaProductosBuscados[iProducto].listaSucursalStocks[
                    iSucursalStocks
                  ].stk_codsuc &&
                isMostrarImput_FacturaTrazablesProvincia(
                  listaSucursalesUnidadesRenglones[iSucursalesUnidadesRenglones]
                    .codSucursal,
                  listaProductosBuscados[iProducto].pro_isTrazable
                ) &&
                isMostrarImput_pedirCC(
                  listaProductosBuscados[iProducto].pro_codtpopro,
                  listaSucursalesUnidadesRenglones[iSucursalesUnidadesRenglones]
                    .codSucursal,
                  listaProductosBuscados[iProducto].listaSucursalStocks
                )
              ) {
                listaSucursalesUnidadesRenglones[
                  iSucursalesUnidadesRenglones
                ].Unidades += nroValor;
                listaSucursalesUnidadesRenglones[
                  iSucursalesUnidadesRenglones
    ...
    for (
      var iSucursal = 0;
      iSucursal < listaSucursalesUnidadesRenglones.length;
      iSucursal++
    ) {
      $("#tdUnidades_" + iSucursal).html(
        listaSucursalesUnidadesRenglones[iSucursal].Unidades
      );
      $("#tdRenglones_" + iSucursal).html(
        listaSucursalesUnidadesRenglones[iSucursal].Renglones
      );

      $("#tdUnidadesCel_" + iSucursal).html(
        listaSucursalesUnidadesRenglones[iSucursal].Unidades
      );
      $("#tdRenglonesCel_" + iSucursal).html(
        listaSucursalesUnidadesRenglones[iSucursal].Renglones
      );
    }
  }
  ...
  if (listaProductosBuscados != null && listaSucursal != null) {
        ...
        if (mytext != null) {
          ...
          if (varValores.length > 0) {
            if (varValores[1] == 0 && varValores[2]) {
              strHtmlMensaje +=
                "<div>" +
                listaProductosBuscados[iProducto].pro_nombre +
                "</div>";
            }
            if (varValores[0] > 0) {
              var proComun = new cProductosAndCantidad();
              proComun.cantidad = varValores[0];
              proComun.codProducto =
                listaProductosBuscados[iProducto].pro_codigo;
              proComun.codProductoNombre =
                listaProductosBuscados[iProducto].pro_nombre;
              proComun.isMostrarTransfersEnClientesPerf =
                listaProductosBuscados[
                  iProducto
                ].isMostrarTransfersEnClientesPerf;
            ...
            if (varValores[1] > 0) {
              ...
              proTransfer.codProducto =
                listaProductosBuscados[iProducto].pro_codigo;
              proTransfer.codProductoNombre =
                listaProductosBuscados[iProducto].pro_nombre;
              proTransfer.codSucursal = listaSucursal[iSucursal];
              proTransfer.isTransferFacturacionDirecta = true;
              proTransfer.isMostrarTransfersEnClientesPerf =
                listaProductosBuscados[
                  iProducto
                ].isMostrarTransfersEnClientesPerf;
              proTransfer.tde_codtfr =
                listaProductosBuscados[iProducto].tde_codtfr;
            ...
            if (varValores[1] == 0 && varValores[2]) {
              ...
              proTransferMsg.codProducto =
                listaProductosBuscados[iProducto].pro_codigo;
              proTransferMsg.pro_nombre =
                listaProductosBuscados[iProducto].pro_nombre;
              proTransferMsg.tde_unidadesbonificadasdescripcion =
                listaProductosBuscados[
                  iProducto
              ...
              proTransferMsg.isMostrarTransfersEnClientesPerf =
                listaProductosBuscados[
                  iProducto
                ].isMostrarTransfersEnClientesPerf;
              proTransferMsg.tde_codtfr =
                listaProductosBuscados[iProducto].tde_codtfr;
            ...
            if (varValores[0] > 0 && varValores[3]) {
              ...
              proTransferMsg.pro_nombre =
                listaProductosBuscados[iProducto].pro_nombre;
              proTransferMsg.pro_ofeunidades =
                listaProductosBuscados[iProducto].pro_ofeunidades;
              proTransferMsg.codSucursal = listaSucursal[iSucursal];
              proTransferMsg.isMostrarTransfersEnClientesPerf =
                listaProductosBuscados[
                  iProducto
    ...
    if (listaProductosAPedir.length > 0) {
      ...
      if (cli_tomaTransfers()) {
        strHtmlMensaje = CargarListaProductosFaltaCantidadParaTransfer(
          listaProductosMostrarTransfer
        );
      }
      if (strHtmlMensaje == "") {
...
function CargarListaProductosFaltaCantidadParaTransfer(
  pListaProductosFaltaCantidadParaTransfer
) {
  var strHtml = "";
  var isMostrar = false;

  strHtml +=
    '<div class="col-xs-12"><div class="alert-success">Productos que no cumplen con la condición mínima de Ofertas/Transfers</div><div class="clear15"></div></div>';
  for (
    ...
    strHtmlSuc +=
      '<th class="col-lg-8 col-md-8 col-sm-8 col-xs-8 text-left">Producto</th>';
    strHtmlSuc +=
      '<th class="col-lg-2 col-md-2 col-sm-2 col-xs-2 text-center">Condición</th>';
    strHtmlSuc +=
    ...
    for (
      var iProductosParaTransfer = 0;
      iProductosParaTransfer < pListaProductosFaltaCantidadParaTransfer.length;
      iProductosParaTransfer++
    ) {
      if (
        pListaProductosFaltaCantidadParaTransfer[iProductosParaTransfer]
          .codSucursal == listaSucursal[iEncabezadoSucursal] &&
        pListaProductosFaltaCantidadParaTransfer[iProductosParaTransfer]
          .isMostrarTransfersEnClientesPerf
      ) {
        ...
        if (
          ...
          strHtmlSuc +=
            '<td class="col-lg-8 col-md-8 col-sm-8 col-xs-8 text-left">';
          strHtmlSuc +=
            pListaProductosFaltaCantidadParaTransfer[iProductosParaTransfer]
              .pro_nombre;
          strHtmlSuc += "</td>";
  ...
  strHtml +=
    '<div class="col-xs-12"><a class="btn_confirmar" href="#"  onclick="modalModuloAlertHide(); CargarPedidoIgualmente(); return false;">CARGAR PEDIDO</a><a class="btn_vaciar" href="#" data-dismiss="modal" onclick="modalModuloAlertHide(); return false;">MODIFICAR PEDIDO</a></div>';
...
function CargarListaSinStock() {
  ...
  for (var i = 0; i < listaProductosBuscados.length; i++) {
    for (
      ...
      for (
        var iSucursal = 0;
        iSucursal < listaProductosBuscados[i].listaSucursalStocks.length;
        iSucursal++
      ) {
        if (
          listaProductosBuscados[i].listaSucursalStocks[iSucursal].stk_codsuc ==
          listaSucursal[iEncabezadoSucursal]
        ) {
          if (
            listaProductosBuscados[i].listaSucursalStocks[
              iSucursal
            ].stk_stock.toLowerCase() == "n"
          ) {
            // Cantidad
            ...
            if (mytext != null) {
              if (
                ...
                if (
                  ...
                  listaSucursalFaltantes[iEncabezadoSucursal].push(
                    objProductoStock
                  );
                }
              }
  ...
  for (
    ...
    strHtmlSuc +=
      '<table class="footable table carrito table-stripped" width="100%" align="center" cellspacing="0" cellpadding="0" border="0"><thead><tr>';
    strHtmlSuc +=
      '<th class="col-lg-8 col-md-8 col-sm-8 col-xs-8 text-left">Nombre producto</th>';
    strHtmlSuc +=
      '<th class="col-lg-2 col-md-2 col-sm-2 col-xs-2 text-center">Cantidad</th>';
    strHtmlSuc += "</tr></thead></table>";
    ...
    for (
      var iProducto = 0;
      iProducto < listaSucursalFaltantes[iEncabezadoSucursal].length;
      iProducto++
    ) {
      var strHtmlColorFondo = "grs";
      if (iProducto % 2 != 0) {
      ...
      strHtmlSuc +=
        '<td class="col-lg-8 col-md-8 col-sm-8 col-xs-8 text-left">';
      strHtmlSuc +=
        listaSucursalFaltantes[iEncabezadoSucursal][iProducto].pro_nombre; //  + '<br/>';
      strHtmlSuc += "</td>";
      strHtmlSuc +=
        '<td class="col-lg-2 col-md-2 col-sm-2 col-xs-2 text-center">';
      for (
        var iSucursalCantidad = 0;
        iSucursalCantidad <
        listaSucursalFaltantes[iEncabezadoSucursal][iProducto]
          .listaSucursalStocks.length;
        iSucursalCantidad++
      ) {
        if (
          listaSucursalFaltantes[iEncabezadoSucursal][iProducto]
            .listaSucursalStocks[iSucursalCantidad].stk_codsuc ===
          listaSucursal[iEncabezadoSucursal]
        ) {
          strHtmlSuc +=
            listaSucursalFaltantes[iEncabezadoSucursal][iProducto]
              .listaSucursalStocks[iSucursalCantidad].cantidadSucursal;
          break;
        }
      }
      //  + '<br/>';
  ...
  strHtml += '<div class="clear"></div>';
  strHtml +=
    '<div class="col-xs-12"><a class="btn_vaciar" href="#" data-dismiss="modal">CERRAR</a></div>';
...
function CargarHtmlHistorialArchivos() {
  ...
  if (listaArchivoSubidos != null) {
    if (listaArchivoSubidos.length > 0) {
      ...
      strHtmlUltimoPedido +=
        '<div class="est_ped st">&Uacute;ltimo pedido realizado:<a class="p_lr" href="#" data-toggle="modal" data-target="#modalSubirarchivo"  onclick="onclickCargar(' +
        listaArchivoSubidos[0].has_id +
        ');return false;">';
      strHtmlUltimoPedido +=
        listaArchivoSubidos[0].has_NombreArchivoOriginal + "</a>";
      var strNombreSucursal0 = "";
      ...
      strHtmlUltimoPedido +=
        '<div class="clear10 visible-xs"></div>' + strNombreSucursal0 + " - ";
      strHtmlUltimoPedido +=
        listaArchivoSubidos[0].has_fechaToString + "</div>";
      ...
      if (listaArchivoSubidos.length > 1) {
        strHtml +=
          ...
          "</div>";
        strHtml +=
          '<table class="footable table sin_b table-stripped" data-empty="No hay informacion disponible" width="100%" align="center" cellspacing="1" cellpadding="5" border="0">';
        strHtml += '<thead class="pagos">';
        strHtml += "<tr>";
        //strHtml += '<th>Archivo</th>';
        strHtml += '<th><table width="100%" cellpadding="0" cellspacing="0">';
        strHtml +=
          '<tr class="hidden-xs"><td class="col-lg-12 text-center">&nbsp;<div class="clear10"></div></td></tr>';
        strHtml +=
          '<tr class="tr_thead"><td class="col-lg-12 text-center">Archivo</td></tr>';
        strHtml += "</table></th>";
        ...
        strHtml +=
          '<tr class="hidden-xs"><td class="col-lg-12 text-center">&nbsp;<div class="clear10"></div></td></tr>';
        strHtml +=
          '<tr class="tr_thead"><td class="col-lg-12 text-center">Sucursal</td></tr>';
        strHtml += "</table></th>";
        //strHtml += '<th>Fecha</th>';
        strHtml += '<th><table width="100%" cellpadding="0" cellspacing="0">';
        strHtml +=
          '<tr class="hidden-xs"><td class="col-lg-12 text-center">&nbsp;<div class="clear10"></div></td></tr>';
        strHtml +=
          '<tr class="tr_thead"><td class="col-lg-12 text-center">Fecha</td></tr>';
        strHtml += "</table></th>";
        strHtml += "</tr>";
        strHtml += "</thead>";
        strHtml += "<tbody>";
        for (var i = 1; i < listaArchivoSubidos.length; i++) {
          ...
          strHtml +=
            '<td onclick="onclickCargar(' +
            listaArchivoSubidos[i].has_id +
            ')" style="color:#065BAB;cursor:pointer;">' +
            listaArchivoSubidos[i].has_NombreArchivoOriginal +
            "</td>";
          ...
          strHtml +=
            "<td>" + listaArchivoSubidos[i].has_fechaToString + "</td>";
          strHtml += "</tr>";
        }
        strHtml += "<tbody>";
        strHtml += "</table>";
