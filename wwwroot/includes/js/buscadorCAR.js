...
var cuerpo_error =
  "Los siguientes productos no se pudieron agregar correctamente:";
var mensajeCantidadSuperaElMaximoParametrizado1 =
  "¿Seguro que desea pedir más de ";
var mensajeCantidadSuperaElMaximoParametrizado2 = " unidades?";
var mensajeCuandoSeMuestraError = "Se produjo un error";
var mensajeTareasMantenimiento =
  "En este momento estamos realizando tareas de mantenimiento, por favor confirme su pedido más tarde.";
...
var htmlAltoCosto =
  '<span class="p_trazable">Alto Costo - Ventas comunicarse al <i class="fa fa-whatsapp linkWP_icon"></i><a class="linkWP" href="tel:3416100365">341 6100365</a></span>';
...
$(document).ready(function () {
  ...
  if (listaTipoEnviosSucursal == null) {
    listaTipoEnviosSucursal = eval(
      "(" + $("#hiddenListaTipoEnviosSucursales").val() + ")"
    );
  ...
  if (listaCadeteriaRestricciones == null) {
    listaCadeteriaRestricciones = eval(
      "(" + $("#hiddenListaCadeteriaRestricciones").val() + ")"
    );
  ...
  if (cantidadMaximaParametrizada == null) {
    cantidadMaximaParametrizada = $(
      "#hiddenCantidadProductoParametrizado"
    ).val();
  ...
  if (listaSucursalesDependienteInfo == null) {
    listaSucursalesDependienteInfo = eval(
      "(" + $("#hiddenListaSucursalesDependienteInfo").val() + ")"
    );
  ...
  if (typeof homeIdOferta == "undefined") {
    homeIdOferta = null;
  } else homeIdOferta = parseInt($("#hiddenHomeIdOferta").val());

  homeIdTransfer = $("#hiddenhomeIdTransfer").val();
  if (typeof homeIdTransfer == "undefined") {
    homeIdTransfer = null;
  } else homeIdTransfer = parseInt($("#hiddenhomeIdTransfer").val());

  homeTipo = $("#hiddenhomeTipo").val();
  if (typeof homeTipo == "undefined") {
    homeTipo = null;
  } else homeTipo = parseInt($("#hiddenhomeTipo").val());

  CargarCarritos();

  if (typeof CargarCarritosTransfersPorSucursal == "function") {
    // isCarritoDiferido == false
    listaCarritoTransferPorSucursal = eval(
      "(" + $("#hiddenListaCarritosTransferPorSucursal").val() + ")"
    );
...
function OnMouseMoveProdructo(e) {
  ...
  $("#divMostradorProducto").css(
    "top",
    top +
      (bt || et) +
      20 -
      $("#divHeader").css("height").replace("px", "") +
      "px"
  ); // this.pageYOffset
  $("#divMostradorProducto").css("left", left + 20 + "px");
}
function OnMouseOverProdructo(pIndice) {
  if ($("#divMostradorProducto").css("display") == "none") {
    LimpiarTimeoutProducto();
    timerProducto = setTimeout(function () {
      AnimarPresentacionProducto(pIndice);
    }, 300);
  }
}
...
function CargarDatosProductos(pIndice) {
  if (listaProductosBuscados[pIndice].pro_nombre != null) {
    $("#tdNombre").html(
      AgregarMark(listaProductosBuscados[pIndice].pro_nombre)
    );
  } else {
    $("#tdNombre").html("");
  }
  if (listaProductosBuscados[pIndice].pro_laboratorio != null) {
    $("#tdLaboratorio").html(
      AgregarMark(listaProductosBuscados[pIndice].pro_laboratorio)
    );
  } else {
    $("#tdLaboratorio").html("");
  }
  if (listaProductosBuscados[pIndice].pro_monodroga != null) {
    $("#tdMonodroga").html(
      AgregarMark(listaProductosBuscados[pIndice].pro_monodroga)
    );
  } else {
    $("#tdMonodroga").html("");
  }
  if (listaProductosBuscados[pIndice].pro_codigobarra != null) {
    $("#tdCodigoBarra").html(
      AgregarMark(listaProductosBuscados[pIndice].pro_codigobarra)
    );
  } else {
    $("#tdCodigoBarra").html("");
  }
  if (listaProductosBuscados[pIndice].pro_codigoalfabeta != null) {
    $("#tdCodigoAlfaBeta").html(
      AgregarMark(listaProductosBuscados[pIndice].pro_codigoalfabeta)
    );
  } else {
    $("#tdCodigoAlfaBeta").html("");
  }
  if (listaProductosBuscados[pIndice].pro_troquel != null) {
    $("#tdTroquel").html(
      AgregarMark(listaProductosBuscados[pIndice].pro_troquel)
    );
  } else {
    $("#tdTroquel").html("");
  }
  ...
  if (listaProductosBuscados[pIndice].pri_nombreArchivo == null) {
    ...
    $("#imgProductoDatos").attr(
      "src",
      "../../servicios/thumbnail?r=" +
        "productos" +
        "&n=" +
        listaProductosBuscados[pIndice].pri_nombreArchivo +
        "&an=" +
        String(250) +
        "&al=" +
        String(250)
  ...
  if (isNotBuscadorEnProceso) {
    ...
    if (varPalabraBuscador !== "") {
      if (varPalabraBuscador.length > 2) {
        ...
        if (
          $("#checkNombre").is(":checked") ||
          $("#checkBoxCodigoBarra").is(":checked") ||
          $("#checkBoxMonodroga").is(":checked") ||
          $("#checkBoxLaboratorio").is(":checked") ||
          $("#checkBoxCodigoAlfaBeta").is(":checked") ||
          $("#checkBoxTroquel").is(":checked") ||
          $("#checkBoxFamilia").is(":checked") ||
          $("#checkBoxTodosOfertas").is(":checked") ||
          isTransferSeleccionado
        ) {
          //
          //Ascender_pro_precio = true;
          //Ascender_PrecioFinal = true;
          ...
          RecuperarProductosVariasColumnas(
            varPalabraBuscador,
            arrayListaColumna,
            $("#checkBoxTodosOfertas").is(":checked"),
            isTransferSeleccionado
...
function getCSSColumnaOrdenar(pValor) {
  if (pValor === -1 && pValor === intColumnaOrdenar) {
    // -1
    if (!Ascender_pro_nombre) return "up";
    else return "down";
  }
  if (pValor === 0 && pValor === intColumnaOrdenar) {
    // 0
    if (!Ascender_pro_precio) return "up";
    else return "down";
  }
  if (pValor === 1 && pValor === intColumnaOrdenar) {
    // 1
    if (!Ascender_PrecioFinal) return "up";
    else return "down";
  }
  if (pValor === 2 && pValor === intColumnaOrdenar) {
    // 2
    if (!Ascender_PrecioConDescuentoOferta) return "up";
    else return "down";
  }
  if (pValor === 3 && pValor === intColumnaOrdenar) {
    // 3
    if (!Ascender_PrecioConTransfer) return "up";
    else return "down";
  }
  if (pValor === 4 && pValor === intColumnaOrdenar) {
    // 3
    if (!Ascender_nroordenamiento) return "up";
    else return "down";
  }
  return "";
}
...
function onclickOrdenarProducto(pValor) {
  ...
  if (pValor === 2) {
    RecuperarProductosOrdenar(
      "PrecioConDescuentoOferta",
      Ascender_PrecioConDescuentoOferta
    );
...
function OnCallBackRecuperarProductosConPaginador(args) {
  ...
  if (mod == 0) {
    ...
    cantPag = Math.ceil(
      varArgs.CantidadRegistroTotal / CantidadFilaPorPagina_const
    );
  }
...
function getHtmlTablaResolucionCelular() {
  ...
  strHtml +=
    '<table class="footable table table-stripped tbl_xs visible-xs' +
    strCssTable +
    '" data-empty="" width="100%" align="center" cellspacing="1" cellpadding="5" border="0">';
  strHtml += "<thead>";
  strHtml += "<tr>";
  //
  ...
  strHtml +=
    '<tbody><tr><td class="col-lg-12 text-center">&nbsp;<div class="clear5"></div></td></tr>';
  strHtml +=
    '<tr class="tr_thead"><td class="col-lg-12 text-left pl click"  title="Ordenar" onclick="onclickOrdenarProducto(-1)">Producto<span class="order ' +
    getCSSColumnaOrdenar(-1) +
    '"></span></td></tr>';
  strHtml += "</tbody></table>";
  strHtml += "</th>";
  ...
  strHtml += '<table width="100%" cellpadding="0" cellspacing="0">';
  strHtml +=
    '<tbody><tr><td colspan="2" class="col-lg-12 text-center">Habitual<div class="clear5"></div></td></tr>';
  strHtml += '<tr class="tr_thead">';
  strHtml +=
    '<td class="col-lg-6 text-center click td_busc_xs"  onclick="onclickOrdenarProducto(0)"><span data-toggle="tooltip" data-placement="bottom" title="Ordenar" data-original-title="Ordenar">Público</span><span class="order ' +
    getCSSColumnaOrdenar(0) +
    '"></span></td>';
  strHtml +=
    '<td class="col-lg-6 text-center click" data-toggle="tooltip" data-placement="bottom" title="Ordenar" data-original-title="Ordenar" onclick="onclickOrdenarProducto(1)">Precio<span class="order ' +
    getCSSColumnaOrdenar(1) +
    '"></span></td>';
  strHtml += "</tr>";
  strHtml += "</tbody></table>";
  ...
  if (listaProductosBuscados.length > 0) {
    for (var i = 0; i < listaProductosBuscados.length; i++) {
      ...
      if (isSubirPedido) {
        ...
        for (
          var iEncabezadoSucursal = 0;
          iEncabezadoSucursal < listaSucursal.length;
          iEncabezadoSucursal++
        ) {
          Log("getHtmlTablaResolucionCelular:" + "0");
          var varValores = CargarProductoCantidadDependiendoTransfer_base(
            i,
            iEncabezadoSucursal,
            listaProductosBuscados[i].cantidad,
            true
          );
          if (
            (varValores[1] == 0 && varValores[2]) ||
            (varValores[0] > 0 && varValores[3])
          ) {
            isRemarcarFila = true;
            break;
          }
        ...
        if (
          isRemarcarFila &&
          cli_tomaTransfers() &&
          listaProductosBuscados[i].isMostrarTransfersEnClientesPerf &&
          listaProductosBuscados[i].isPermitirPedirProducto
        ) {
      ...
      strHtml +=
        '<td class="col-xs-8 text-left"  onclick="detalleProducto_celular(' +
        i +
        '); return false;" ><a class="prod_xs" href="" data-toggle="modal" data-target="#modalProd_xs_1"  >';
      ...
      if (isNotGLNisTrazable) {
        strHtml +=
          '<span class="p_trazable">Producto trazable. Farmacia sin GLN.</span>';
      }
      if (
        cli_tomaTransfers() &&
        listaProductosBuscados[i].isMostrarTransfersEnClientesPerf
        ...
        if (listaProductosBuscados[i].pro_vtasolotransfer) {
          strHtml +=
            '<span class="p_trazable">Se vende solo por transfer</span>'; // '<span class="spanProductoTrazableCLiSinGLN" >&nbsp;&nbsp;&nbsp;Se vende solo por transfer</span>';
        }
      }
      // Alto Costo
      ...
      if (
        listaProductosBuscados[i].pro_vtasolotransfer &&
        !isMostrarSoloTransfer_FacturacionDirecta(listaProductosBuscados[i])
      ) {
      ...
      var precioPublico =
        "$&nbsp;" +
        FormatoDecimalConDivisorMiles(
          listaProductosBuscados[i].pro_precio.toFixed(2)
        );
      ...
      var precioHabitual =
        "$&nbsp;" +
        FormatoDecimalConDivisorMiles(
          listaProductosBuscados[i].PrecioFinal.toFixed(2)
        );
      if (listaProductosBuscados[i].pro_nombre.match("^PAÑAL PAMI AD")) {
        precioHabitual =
          "$&nbsp;" +
          FormatoDecimalConDivisorMiles(
            listaProductosBuscados[i].pro_preciofarmacia.toFixed(2)
          );
      }
      if (listaProductosBuscados[i].pro_AltoCosto) {
        precioHabitual = "";
    ...
    strHtml +=
      '<tr><td colspan="8" class="text-center"><p class="color_red">La búsqueda no arroja resultados</p></td></tr>';
  }
  strHtml += "</tbody>";
  ...
  if (isSubirPedido) {
    ...
    for (
      ...
      strHtml_cel_part_1 +=
        '<td colspan="2" class="col-xs-10 text-right">Renglones ' +
        listaSucursal[iEncabezadoSucursal] +
        ":</td>";
      strHtml_cel_part_1 +=
        '<td id="tdRenglonesCel_' +
        iEncabezadoSucursal +
        '" class="col-xs-2 text-center pad_7"></td>';
      strHtml_cel_part_1 += "</tr>";
      strHtml_cel_part_2 += "<tr>";
      strHtml_cel_part_2 +=
        '<td colspan="2" class="col-xs-10 text-right">' +
        textUnidades +
        listaSucursal[iEncabezadoSucursal] +
        ":</td>";
      strHtml_cel_part_2 +=
        '<td id="tdUnidadesCel_' +
        iEncabezadoSucursal +
        '" class="col-xs-2 text-center pad_7"></td>';
      strHtml_cel_part_2 += "</tr>";
    }
    ...
    strHtml +=
      ...
      strCssTable +
      '"><a class="btn_emp float-right" href="#" data-toggle="modal" data-target="#modalCargarPedido" onclick="CargarPedido(); return false;">CARGAR PEDIDO</a></div>';
  } else {
    strHtml += "</table>";
  }
  return strHtml;
...
function ObtenerCantidadProductoMasTransfer(
  pIdSucursal,
  pIdProduco,
  pNombreProducto
) {
  ...
  var resultadoTransfer = ObtenerCantidadProductoTransfer(
    pIdSucursal,
    pNombreProducto
  );
  ...
  if (listaCarritos !== null) {
    for (var i = 0; i < listaCarritos.length; i++) {
      if (listaCarritos[i].codSucursal === pIdSucursal) {
        for (
          var iProducto = 0;
          iProducto < listaCarritos[i].listaProductos.length;
          iProducto++
        ) {
          if (
            listaCarritos[i].listaProductos[iProducto].codProducto ===
            pIdProduco
          ) {
...
function ObtenerCantidadProductoTransfer(pIdSucursal, pNombreProducto) {
  ...
  if (listaCarritoTransferPorSucursal !== null) {
    if (listaCarritoTransferPorSucursal.length > 0) {
      for (
        var iSucursal = 0;
        iSucursal < listaCarritoTransferPorSucursal.length;
        iSucursal++
      ) {
        if (
          listaCarritoTransferPorSucursal[iSucursal].Sucursal === pIdSucursal
        ) {
          for (
            var iTransfer = 0;
            iTransfer <
            listaCarritoTransferPorSucursal[iSucursal].listaTransfer.length;
            iTransfer++
          ) {
            for (
              var iTransferProductos = 0;
              iTransferProductos <
              listaCarritoTransferPorSucursal[iSucursal].listaTransfer[
                iTransfer
              ...
              if (
                listaCarritoTransferPorSucursal[iSucursal].listaTransfer[
                  iTransfer
                ].listaProductos[iTransferProductos].tde_codpro ===
                pNombreProducto
              ) {
                if (
                  listaCarritoTransferPorSucursal[iSucursal].listaTransfer[
                    iTransfer
                  ].listaProductos[iTransferProductos]
                    .isProductoFacturacionDirecta
                ) {
                  resultado +=
                    listaCarritoTransferPorSucursal[iSucursal].listaTransfer[
                      iTransfer
                    ].listaProductos[iTransferProductos].cantidad;
                }
              }
            }
          }
...
function onblurSucursal_base(pCantidad, pFila, pColumna) {
  ...
  if (isNotNullEmpty(pCantidad)) {
    ...
    if (isSubirPedido) isPasarBase = true;
    else {
      // Calcular si producto transfer
      var cantidadComparativa = ObtenerCantidadProductoMasTransfer(
        listaSucursal[columna],
        listaProductosBuscados[fila].pro_codigo,
        listaProductosBuscados[fila].pro_nombre
      );
      if (cantidadComparativa != pCantidad) isPasarBase = true;
    }
    if (isPasarBase) {
      ...
      if (isNotMaximaCantidadSuperada) {
        ...
        if (isExcedeImporte) {
          ...
          var isValidarSoloTransferFacturacionDirecta =
            funValidarSoloTransferFacturacionDirecta(
              listaProductosBuscados[fila],
              listaSucursal[columna],
              pCantidad,
              true
            );
          if (isValidarSoloTransferFacturacionDirecta) {
            ...
            if (isSubirPedido) {
              ...
              var resultadoCantidadCambio =
                CargarProductoCantidadDependiendoTransfer(
                  fila,
                  columna,
                  pCantidad
                );
        ...
        mensaje_informacion(
          MostrarTextoSuperaCantidadMaxima(
            listaProductosBuscados[fila].pro_nombre,
            listaProductosBuscados[fila].pro_canmaxima
          )
        );
        var cantidadAnterior = ObtenerCantidadProducto(
          listaSucursal[columna],
          listaProductosBuscados[fila].pro_codigo
        );
    ...
    if (isSubirPedido) {
      ...
      var cantidad = ObtenerCantidadProducto(
        listaSucursal[columna],
        listaProductosBuscados[fila].pro_codigo
      );
      if (cantidad != "") {
  ...
  if (isSubirPedido) {
    ...
    if (
      isCambioValor_SubirPedido &&
      listaProductosBuscados[fila].isPermitirPedirProducto &&
      listaProductosBuscados[fila].isMostrarTransfersEnClientesPerf
    ) {
      var isRemarcarFila = false;
...
function isMostrarSoloTransfer_FacturacionDirecta(pProducto) {
  var result = false;
  if (
    pProducto.pro_vtasolotransfer != null &&
    pProducto.pro_vtasolotransfer == 1 &&
    pProducto.tfr_facturaciondirecta != null &&
    pProducto.tfr_facturaciondirecta == 1
  ) {
    result = true;
  }
  return result;
}
function funValidarSoloTransferFacturacionDirecta(
  pProducto,
  pIdSucursal,
  pCantidad,
  pIsDesdeBuscador
  ...
  var result = true;
  if (
    isSoloTransferFacturacionDirecta(
      pProducto,
      pIdSucursal,
      pCantidad,
      pIsDesdeBuscador
    ...
    var htmlMensaje =
      ...
      cuerpo_error +
      "</p><ul><li>" +
      pProducto.pro_nombre +
      "</li></ul>";
    mensaje_transfer(titulo_transferDirecto, htmlMensaje);
  }
  return result;
}
function isSoloTransferFacturacionDirecta(
  pProducto,
  pIdSucursal,
  pCantidad,
  pIsDesdeBuscador
  ...
  if (
    ...
    var cantidadCarritoComun =
      isSoloTransferFacturacionDirecta_logica_return_CantidadCarritoComun(
        pProducto,
        pIdSucursal,
        pCantidad,
        false
...
function isSoloTransferFacturacionDirecta_logica_return_CantidadCarritoComun(
  pProducto,
  pIdSucursal,
  pCantidad,
  pIsSubirPedido
  ...
  if (pProducto.isProductoFacturacionDirecta) {
    ...
    var precioConDescuentoDependiendoCantidad =
      CalcularPrecioProductosEnCarrito(
        pProducto.PrecioFinal,
        pCantidad,
        pProducto.pro_ofeunidades,
        pProducto.pro_ofeporcentaje
      );
    if (pCantidad == 0) {
      //
    } else {
      precioConDescuentoDependiendoCantidad =
        precioConDescuentoDependiendoCantidad / pCantidad;
    }
    if (
      parseFloat(precioConDescuentoDependiendoCantidad) >=
      parseFloat(pProducto.PrecioFinalTransfer)
    ) {
      var isSumarTransfer = false;

      if (
        pProducto.tde_muluni != null &&
        pProducto.tde_unidadesbonificadas != null
      ) {
        ProductoIsTransferDirecto_subirPedido = true;
        /// UNIDAD MULTIPLO Y BONIFICADA
        if (
          pCantidad >= pProducto.tde_muluni &&
          pCantidad <= pProducto.tde_muluni + pProducto.tde_unidadesbonificadas
        ) {
          // es multiplo
          isSumarTransfer = true;
          cantidadCarritoTransfer =
            pProducto.tde_muluni + pProducto.tde_unidadesbonificadas;
        } else if (
          pCantidad >
          pProducto.tde_muluni + pProducto.tde_unidadesbonificadas
        ) {
          ...
          for (
            var iCantMulti = 0;
            iCantMulti < cantidadMultiplicar;
            iCantMulti++
          ) {
            var cantTemp =
              iCantMulti *
          ...
          if (cantidadCarritoTransfer == pCantidad) {
            ...
            if (
              cantidadCarritoComun >= pProducto.tde_muluni &&
              cantidadCarritoComun <=
                pProducto.tde_muluni + pProducto.tde_unidadesbonificadas
            ) {
              cantidadCarritoTransfer +=
                pProducto.tde_muluni + pProducto.tde_unidadesbonificadas;
        ...
        if (
          pProducto.tde_minuni <= pCantidad &&
          pProducto.tde_maxuni >= pCantidad
        ) {
          isSumarTransfer = true;
  ...
  if (isPasarDirectamente) {
    ...
    if (!pIsSubirPedido) {
      var cantidadTransfer = ObtenerCantidadProductoTransfer(
        pIdSucursal,
        pProducto.pro_nombre
      );
      if (cantidadTransfer != "") {
  ...
  if (pIsSubirPedido) {
    ...
    resultadoReturn_subirPedido.push(
      IsProductoMostrarFaltaCantidadOferta_subirPedido
    );
...
function funMostrarMensajeCantidadSuperada() {
  isMoverCursor = false;
  mensaje_confirmar(
    mensajeCantidadSuperaElMaximoParametrizado1 +
      cantidadMaximaParametrizada +
      mensajeCantidadSuperaElMaximoParametrizado2,
    "funExcedeImporteAceptar(); return false;",
    "funExcedeImporteCancelar(); return false;"
  );
}
function funExcedeImporteAceptar() {
  if (
    isNotNullEmpty(ExcedeImporteFila) &&
    isNotNullEmpty(ExcedeImporteColumna) &&
    isNotNullEmpty(ExcedeImporteValor)
  ) {
    if (isSubirPedido) {
      Log("funExcedeImporteAceptar:" + "1");
      AgregarAlHistorialProductoCarrito(
        ExcedeImporteFila,
        ExcedeImporteColumna,
        ExcedeImporteValor,
        true
      );
    } else {
      Log("funExcedeImporteAceptar:" + "0");
      var resultadoCantidadCambio = CargarProductoCantidadDependiendoTransfer(
        ExcedeImporteFila,
        ExcedeImporteColumna,
        ExcedeImporteValor
      );
      if (ExcedeImporteValor != resultadoCantidadCambio) {
        genericInputSucValue(
          ExcedeImporteFila,
          ExcedeImporteColumna,
          resultadoCantidadCambio
        );
    ...
    if (isEnterExcedeImporte) {
      ...
      if (
        ...
        genericInputSucSelect(
          ExcedeImporteSiguienteFila,
          ExcedeImporteSiguienteColumna
        );
...
function CargarProductoCantidadDependiendoTransfer(pFila, pColumna, pCantidad) {
  ...
  return CargarProductoCantidadDependiendoTransfer_base(
    pFila,
    pColumna,
    pCantidad,
    false
...
function CargarProductoCantidadDependiendoTransfer_base(
  pFila,
  pColumna,
  pCantidad,
  pIsSubirPedido
  ...
  if (listaProductosBuscados[pFila].isProductoFacturacionDirecta) {
    // facturacion directa
    // Combiene transfer o promocion
    var precioConDescuentoDependiendoCantidad =
      CalcularPrecioProductosEnCarrito(
        listaProductosBuscados[pFila].PrecioFinal,
        pCantidad,
        listaProductosBuscados[pFila].pro_ofeunidades,
        listaProductosBuscados[pFila].pro_ofeporcentaje
      );
    if (pCantidad == 0) {
      //
    } else {
    ...
    if (
      parseFloat(precioConDescuentoDependiendoCantidad) >=
      parseFloat(listaProductosBuscados[pFila].PrecioFinalTransfer)
    ) {
      var isSumarTransfer = false;

      if (
        listaProductosBuscados[pFila].tde_muluni != null &&
        listaProductosBuscados[pFila].tde_unidadesbonificadas != null
      ) {
        ProductoIsTransferDirecto_subirPedido = true;
        /// UNIDAD MULTIPLO Y BONIFICADA
        if (
          pCantidad >= listaProductosBuscados[pFila].tde_muluni &&
          pCantidad <=
            listaProductosBuscados[pFila].tde_muluni +
              listaProductosBuscados[pFila].tde_unidadesbonificadas
        ) {
          // es multiplo
          isSumarTransfer = true;
          cantidadCarritoTransfer =
          ...
          isSumarTransfer = true;
          var cantidadMultiplicar = parseInt(
            pCantidad / listaProductosBuscados[pFila].tde_muluni
          );
          cantidadCarritoTransfer =
            cantidadMultiplicar *
        ...
        if (isSumarTransfer) {
          if (!pIsSubirPedido) {
            ...
            var tempListaProductos = [];
            var objProducto = new jcTransfersProductos();
            objProducto.codProductoNombre =
              listaProductosBuscados[pFila].tde_codpro; // Para la funcion en el servidor
            ...
            AgregarProductosTransfersAlCarrito(
              tempListaProductos,
              listaProductosBuscados[pFila].tde_codtfr,
              listaSucursal[pColumna],
              "OnCallBackAgregarProductosTransfersAlCarritoDesdeBuscador"
            );
            if (cantidadCarritoComun == 0) {
              var cantidad = ObtenerCantidadProducto(
                listaSucursal[pColumna],
                listaProductosBuscados[pFila].pro_codigo
              );
              if (cantidad != "") {
        ...
        if (pCantidad == listaProductosBuscados[pFila].tde_fijuni) {
          ...
          cantidadCarritoTransfer = listaProductosBuscados[pFila].tde_fijuni;
          cantidadCarritoComun =
            pCantidad - listaProductosBuscados[pFila].tde_fijuni;
        }
        if (isSumarTransfer) {
          if (!pIsSubirPedido) {
            // sumar a transfer
        ...
        listaProductosBuscados[pFila].tde_minuni != null &&
        listaProductosBuscados[pFila].tde_maxuni != null
      ) {
        ProductoIsTransferDirecto_subirPedido = true;
        // UNIDAD MAXIMA Y MINIMA
        if (
          listaProductosBuscados[pFila].tde_minuni <= pCantidad &&
          listaProductosBuscados[pFila].tde_maxuni >= pCantidad
        ) {
          isSumarTransfer = true;
          cantidadCarritoTransfer = pCantidad;
          ...
          cantidadCarritoComun =
            pCantidad - listaProductosBuscados[pFila].tde_maxuni;
        }
        if (isSumarTransfer) {
          if (!pIsSubirPedido) {
            // sumar a transfer
  ...
  if (isPasarDirectamente) {
    ...
    if (!pIsSubirPedido) {
      ...
      var cantidadTransfer = ObtenerCantidadProductoTransfer(
        listaSucursal[pColumna],
        listaProductosBuscados[pFila].pro_nombre
      ); // ObtenerCantidadProducto(listaSucursal[pColumna], listaProductosBuscados[pFila].pro_codigo);
      if (cantidadTransfer != "") {
        //AgregarAlHistorialProductoCarrito(pFila, pColumna, 0, true);
        var tempListaProductos = [];
        var objProducto = new jcTransfersProductos();
  ...
  if (pIsSubirPedido) {
    ...
    resultadoReturn =
      parseInt(cantidadCarritoTransfer) + parseInt(cantidadCarritoComun);
    return resultadoReturn;
  }
}
...
function OnCallBackRecuperarProductos(args) {
  if (args !== null) {
    if (args !== "") {
      ...
      var cssTh_cabecera =
        ' class="col-lg-3 col-md-3 col-sm-3 col-xs-9 text-left no-padding" ';
      ...
      switch (listaSucursal.length) {
        case 1:
          cssTh_cabecera =
            ' class="col-lg-5 col-md-5 text-left th1ColumnaCabecera"  ';
          cssTd_cabeceraBody =
            " col-lg-5 col-md-5 text-left th1ColumnaCabeceraBody ";
        ...
        case 2:
          cssTh_cabecera =
            ' class="col-lg-4 col-md-4 col-sm-3 col-xs-9 text-left th1ColumnaCabecera"  ';
          cssTd_cabeceraBody =
            " col-lg-4 col-md-4 col-sm-3 text-left th1ColumnaCabeceraBody ";
          cssFootTd1 = " col-lg-9 col-md-9 ";
          cssFootTd2 = " col-lg-1 col-md-1 col-sm-1 text-center ";
          cssFootTd3 =
            " col-lg-3 col-md-3 col-sm-1 col-xs-1 text-center no-padding ";
          cssFootTd3Div = " col-xs-6 col_small flex_8 ";
          break;
        case 3:
          cssTh_cabecera =
            ' class="col-lg-3 col-md-3 col-sm-3 col-xs-9 text-left th1ColumnaCabecera"  ';
          cssTd_cabeceraBody =
            " col-lg-3 col-md-3 col-sm-3 text-left th1ColumnaCabeceraBody ";
        ...
        case 4:
          cssTh_cabecera =
            ' class="col-lg-2 col-md-2 col-sm-2 col-xs-9 text-left th1ColumnaCabecera"  ';
          cssTd_cabeceraBody =
            " col-lg-3 col-md-3 col-sm-3 text-left th1ColumnaCabeceraBody ";
          cssFootTd1 = " col-lg-8 col-md-8 ";
          cssFootTd2 = " col-lg-1 col-md-1 col-sm-1 text-center ";
      ...
      if (isSubirPedido) {
        ...
        strHtml +=
          '<table class="footable table table-stripped" width="100%" align="center" cellspacing="1" cellpadding="5" border="0">';
        classSubirPedidoTheadTr1 = ' style="border-top: none !important" ';
      } else {
        strHtml +=
          '<table class="footable table table-stripped hidden-xs" width="100%" align="center" cellspacing="1" cellpadding="5" border="0">';
      }
      strHtml += "<thead>";
      ...
      if (isSubirPedido) {
        strHtml += '<table width="100%" cellpadding="0" cellspacing="0">';
        strHtml +=
          '<tr><td colspan="2" class="col-lg-12 text-center">&nbsp;<div class="clear5"></div></td></tr>';
        strHtml += '<tr class="tr_thead">';
        strHtml +=
          '<td class="col-lg-10 col-md-10 pl text-left no_border click"><span data-toggle="tooltip" data-placement="bottom" title="Ordenar" onclick="onclickOrdenarProducto(-1)">Producto</span><span id="spanOrder_1" class="order ' +
          getCSSColumnaOrdenar(-1) +
          '"></span></td>';
        strHtml +=
          '<td class="col-lg-2 text-center click no-padding hidden-md" data-toggle="tooltip" data-placement="bottom" title="Ordenar" onclick="onclickOrdenarProducto(4)">Orden <span id="spanOrder4" class="order ' +
          getCSSColumnaOrdenar(4) +
          '"></span></td>';
        strHtml +=
          '<td class="col-md-2 text-center click no-padding visible-md" data-toggle="tooltip" data-placement="bottom" onclick="onclickOrdenarProducto(4)">Or <span id="spanOrder4b" class="order ' +
          getCSSColumnaOrdenar(4) +
          '"></span></td>';
        strHtml += "</tr>";
        strHtml += "</table>";
        ...
        strHtml += '<table width="100%" cellpadding="0" cellspacing="0">';
        strHtml +=
          '<tr><td class="col-lg-12 text-center">&nbsp;<div class="clear5"></td></tr>';
        strHtml +=
          '<tr class="tr_thead"><td class="col-lg-12 text-left pl click" data-toggle="tooltip" data-placement="bottom" title="Ordenar" onclick="onclickOrdenarProducto(-1)">Producto <span id="spanOrder_1" class="order ' +
          getCSSColumnaOrdenar(-1) +
          '"></span></td></tr>';
        strHtml += "</table>";
      }
      ...
      strHtml +=
        '<th class="col-lg-2 col-md-2 col-sm-2  text-center" colspan="2">';
      strHtml += '<table width="100%" cellpadding="0" cellspacing="0">';
      strHtml +=
        '<tr><td colspan="2" class="col-lg-12 text-center">Habitual<div class="clear5"></div></td></tr>';
      strHtml += '<tr class="tr_thead">';
      strHtml +=
        '<td class="col-lg-6 text-center click no_border" onclick="onclickOrdenarProducto(0)"><span data-toggle="tooltip" data-placement="bottom" title="Ordenar">P&uacute;blico</span><span id="spanOrder0" class="order ' +
        getCSSColumnaOrdenar(0) +
        '"></span></td>';
      strHtml +=
        '<td class="col-lg-6 text-center click"  data-toggle="tooltip" data-placement="bottom" title="Ordenar" onclick="onclickOrdenarProducto(1)">Precio<span id="spanOrder1" class="order ' +
        getCSSColumnaOrdenar(1) +
        '"></span></td>';
      strHtml += "</tr>";
      strHtml += "</table>";
      ...
      //
      strHtml +=
        '<th class="col-lg-2 col-md-2 col-sm-2 text-center no-padding">';
      strHtml += '<table width="100%" cellpadding="0" cellspacing="0">';
      strHtml +=
        '<tr><td colspan="3" class="col-lg-12 text-center">Oferta<div class="clear5"></div></td></tr>';
      strHtml += '<tr class="tr_thead">';
      strHtml +=
        '<td class="col-lg-3 col-md-3 col-sm-3 text-center no_border">% PVP</td>';
      strHtml +=
        '<td class="col-lg-3 col-md-3 col-sm-3 text-center no_border">Min</td>';
      strHtml +=
        '<td class="col-lg-6 col-md-6 col-sm-6 click text-center" data-toggle="tooltip" data-placement="bottom" title="Ordenar" onclick="onclickOrdenarProducto(2)">Precio<span id="spanOrder2" class="order ' +
        getCSSColumnaOrdenar(2) +
        '"></span></td>';
      strHtml += "</tr>";
      strHtml += "</table>";
      strHtml += "</th>";
      ...
      strHtml += '<table width="100%" cellpadding="0" cellspacing="0">';
      strHtml +=
        '<tr><td colspan="3" class="col-lg-12 text-center">Transfer<div class="clear5"></div></td></tr>';
      strHtml += '<tr class="tr_thead">';
      strHtml +=
        '<td class="col-lg-' +
        col_lg_transfer_pvp +
        ...
        " col-sm-" +
        col_sm_transfer_pvp +
        '  text-center no_border">% PVP</td>';
      strHtml +=
        '<td class="col-lg-' +
        col_lg_transfer_cond +
        " col-md-" +
        col_md_transfer_cond +
        " col-sm-" +
        col_sm_transfer_cond +
        '  text-center no_border">Cond</td>';
      strHtml +=
        '<td class="col-lg-' +
        col_lg_transfer_precio +
        " col-md-" +
        col_md_transfer_precio +
        " col-sm-" +
        col_sm_transfer_precio +
        '  text-center click" data-toggle="tooltip" data-placement="bottom" title="Ordenar"  onclick="onclickOrdenarProducto(3)">Precio<span  id="spanOrder3" class="order ' +
        getCSSColumnaOrdenar(3) +
        '"></span></td>';
      strHtml += "</tr>";
      strHtml += "</table>";
      strHtml += "</th>";
      ...
      for (
        ...
        strHtml +=
          '<th class="col-lg-1 col-md-1 col-sm-1 col-xs-1 text-center no-padding">';
        strHtml += '<table width="100%" cellpadding="0" cellspacing="0">';
        strHtml +=
          '<tr><td class="col-lg-12 text-center ">&nbsp;<div class="clear5"></div></td></tr>';
        strHtml +=
          '<tr class="tr_thead"><td class="col-lg-12 text-center">' +
          ConvertirSucursalParaColumna(listaSucursal[iEncabezadoSucursal]) +
          "</td></tr>";
        strHtml += "</table>";
        strHtml += "</th>";
      }
      ...
      if (isSubirPedido) {
        ...
        strHtml +=
          '<table class="footable table table-stripped mb_0" width="100%" align="center" cellspacing="1" cellpadding="5" border="0">';
      }
      /////////////////////////////
      ...
      if (listaProductosBuscados.length > 0) {
        for (var i = 0; i < listaProductosBuscados.length; i++) {
          ...
          if (
            !isMostrarImput_AceptaPsicotropicos(
              listaProductosBuscados[i].pro_codtpovta
            )
          ) {
            isMostrarImput = false;
          }
          ...
          strHtml +=
            '<tr class="' +
            strTrColorFondo +
            " cssFilaBuscadorDesmarcar cssFilaBuscador_" +
            i +
            '">';
          var strHtmlColorFondo = "";
          var tdBodyClass = "";
          ...
          strHtml +=
            '<td class="' + tdBodyClass + " tdNombreProducto tdSeparadorGrilla";
          strHtml += strHtmlColorFondo + '" '; //'">';
          strHtml +=
            ' OnMouseMove="OnMouseMoveProdructo(event)" OnMouseOver="OnMouseOverProdructo(' +
            i +
            ')" OnMouseOut="OnMouseOutProdructo()"; return false; style="width: 200px;" >';
          if (isSubirPedido) {
            strHtml +=
              '<div class="col-lg-10 col-md-10 col-sm-10 div_prod_sp">';
          }
          strHtml += listaProductosBuscados[i].pro_nombre; //AgregarMark( listaProductosBuscados[i].pro_nombre);
          if (isNotGLNisTrazable) {
            strHtml +=
          ...
          if (
            ...
            if (listaProductosBuscados[i].isTieneTransfer) {
              strHtml +=
                '<button class="btn-product" style="margin: 5px;" onclick="onclickRecuperarTransfer(' +
                i +
                '); return false;">► Ver Combos'; // Utilizamos un botón en lugar de un div
              if (isSubirPedido) {
                strHtml +=
                  '<div class="col-lg-10 col-md-10 col-sm-10 div_prod_sp">';
              }

              strHtml += "</button>"; // Cerramos el botón //'<span>&nbsp;&nbsp;&raquo;&nbsp;Transfer Combo</span>';
            } else if (listaProductosBuscados[i].isProductoFacturacionDirecta) {
              strHtml += '<span class="trf">TRF</span>'; //'<span>&nbsp;&nbsp;&raquo;&nbsp;TRF</span>';
            }
            // Mostrar solo producto Transfer
          ...
          if (listaProductosBuscados[i].pri_nombreArchivo !== null) {
            //onclickAmpliarImagen(\'' + listaProductosBuscados[i].pri_nombreArchivo + '\')
            strHtml +=
              '<i class="fa fa-camera color_emp_st pull-right" onclick="onclickAmpliarImagen(' +
              i +
          ...
          if (isSubirPedido) {
            strHtml += "</div>";
            strHtml +=
              '<div class="col-lg-2 col-md-2 col-sm-2 col_small">' +
              listaProductosBuscados[i].nroordenamiento +
              "</div>"; //&nbsp;
          }
          ...
          strHtml +=
            '<td class="col-lg-1 col-md-1 col-sm-1 text-center tdSeparadorGrilla">' +
            precioHabitual +
            "</td>";
          ...
          if (cli_tomaOfertas()) {
            if (
              listaProductosBuscados[i].pro_ofeunidades !== 0 ||
              listaProductosBuscados[i].pro_ofeporcentaje !== 0
            ) {
              varOfeunidades = listaProductosBuscados[i].pro_ofeunidades;
              //varOfeporcentaje = listaProductosBuscados[i].pro_ofeporcentaje;
              varOfeporcentaje = FormatoDecimalConDivisorMiles(
                getPorcDtoSobreOferta(i).toFixed(2)
              );
              varPrecioConDescuentoOferta =
                "$&nbsp;" +
                FormatoDecimalConDivisorMiles(
                  listaProductosBuscados[i].PrecioConDescuentoOferta.toFixed(2)
                );
            }
          }
          //Oferta
          strHtml +=
            '<td class="col-lg-2 col-md-2 col-sm-2 text-center no-padding tdSeparadorGrilla">';
          strHtml +=
            '<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 col_small">' +
            varOfeporcentaje +
            "</div>";
          strHtml +=
            '<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 col_small">' +
            ...
            "</div>";
          strHtml +=
            '<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 col_small">' +
            varPrecioConDescuentoOferta +
            "</div>";
          strHtml += "</td>";
          // FIn Oferta
          ...
          if (
            ...
            if (listaProductosBuscados[i].isProductoFacturacionDirecta) {
              if (
                listaProductosBuscados[i].tde_unidadesbonificadasdescripcion !==
                null
              ) {
                varTransferFacturacionDirectaCondicion =
                  listaProductosBuscados[i].tde_unidadesbonificadasdescripcion;
              }
              if (listaProductosBuscados[i].PrecioFinalTransfer !== null) {
                varTransferFacturacionDirectaPrecio =
                  "$&nbsp;" +
                  FormatoDecimalConDivisorMiles(
                    listaProductosBuscados[i].PrecioFinalTransfer.toFixed(2)
                  );
              }
            }
          }
          ...
          strHtml +=
            '<td colspan="2" class="col-lg-2 col-md-2 col-sm-2 text-center no-padding tdSeparadorGrilla">';
          strHtml +=
            '<div class="col-lg-' +
            col_lg_transfer_pvp +
            " col-md-" +
          ...
          if (listaProductosBuscados[i].tde_PorcDtoSobrePVP !== null) {
            strHtml += FormatoDecimalConDivisorMiles(
              getPorcDtoSobrePVP(i).toFixed(2)
            );
          }
          strHtml += "</div>";
          //
          ...
          if (varTransferFacturacionDirectaCondicion !== "") {
            strHtml +=
              '<div  OnMouseMove="OnMouseMoveProdructoFacturacionDirecta(event)" OnMouseOver="OnMouseOverProdructoFacturacionDirecta(' +
              i +
              ')" OnMouseOut="OnMouseOutProdructoFacturacionDirecta()"  >';
          ...
          for (
            ...
            if (intPaginadorTipoDeRecuperar !== 2) {
              // todo transfer
              isDibujarStock = true;
            } // fin      if (intPaginadorTipoDeRecuperar != 2) {
            else {
              // si selecciona todo transfer y es producto facturacion directa
              //if (!isCarritoDiferido) {
              if (listaProductosBuscados[i].isProductoFacturacionDirecta) {
                isDibujarStock = true;
              }
            ...
            if (isDibujarStock) {
              for (
                var iSucursal = 0;
                iSucursal <
                listaProductosBuscados[i].listaSucursalStocks.length;
                iSucursal++
              ) {
                if (
                  listaProductosBuscados[i].listaSucursalStocks[iSucursal]
                    .stk_codsuc === listaSucursal[iEncabezadoSucursal]
                ) {
                  ...
                  strHtml +=
                    ...
                    '"></div>';
                  if (isMostrarImput) {
                    is_pedirCC = isMostrarImput_pedirCC(
                      listaProductosBuscados[i].pro_codtpopro,
                      listaSucursal[iEncabezadoSucursal],
                      listaProductosBuscados[i].listaSucursalStocks
                    );
                  }
                  if (isMostrarImput) {
                    //(pIndexSucursal, pIndexProducto)
                    isMostrarImputSucursal =
                      isMostrarImput_FacturaTrazablesProvincia(
                        listaSucursal[iEncabezadoSucursal],
                        listaProductosBuscados[i].pro_isTrazable
                      );
                  }
                  if (
                    isMostrarImput &&
                    listaProductosBuscados[i].pro_codtpopro == "P"
                  ) {
                    isMostrarImputPerfu = MostrarImputPerfu(
                      listaProductosBuscados[i].pro_codtpopro,
                      listaSucursal[iEncabezadoSucursal],
                      listaProductosBuscados[i].listaSucursalStocks
                    );
                    ...
                    if (isSubirPedido) {
                      ...
                      cantidadDeProductoEnCarrito =
                        listaProductosBuscados[i].listaSucursalStocks[iSucursal]
                          .cantidadSucursal;
                      if (
                        cantidadDeProductoEnCarrito == 0 &&
                        !isMostrarImput_pedirCC(
                          listaProductosBuscados[i].pro_codtpopro,
                          "CC",
                          listaProductosBuscados[i].listaSucursalStocks
                        )
                      ) {
                        var cantidadDeProductoEnCarrito_temp =
                          getCantidad_SubirArchivo_pedirCC(
                            listaProductosBuscados[i].pro_codtpopro,
                            listaSucursal[iEncabezadoSucursal],
                            listaProductosBuscados[i].listaSucursalStocks
                          );
                        if (isNotNullEmpty(cantidadDeProductoEnCarrito_temp)) {
                          cantidadDeProductoEnCarrito =
                            cantidadDeProductoEnCarrito_temp;
                        }
                      }
                    } else {
                      cantidadDeProductoEnCarrito =
                        ObtenerCantidadProductoMasTransfer(
                          listaSucursal[iEncabezadoSucursal],
                          listaProductosBuscados[i].pro_codigo,
                          listaProductosBuscados[i].pro_nombre
                        );
                    }
                    var typeInput = ' type="text" ';
                    if (isMobile()) typeInput = ' type="number" ';
                    strHtml +=
                      '<input class="form-shop cssFocusCantProdCarrito" id="inputSuc' +
                      i +
                      "_" +
                      iEncabezadoSucursal +
                      '" ' +
                      typeInput +
                      ' onfocus="onfocusSucursal(this)" onblur="onblurSucursal(this)" onkeypress="return onKeypressCantProductos(event)" value="' +
                      cantidadDeProductoEnCarrito +
                      '" ></input>';
                  }
                  //strHtml += '</div>';
      ...
      if (isSubirPedido) {
        ...
        strHtml +=
          '<table class="footable table table-stripped" width="100%" align="center" cellspacing="0" cellpadding="0" border="0">';
        ...
        strHtml += "</div>";
        strHtml +=
          '<div><a class="btn_emp float-right" href="#" data-toggle="modal" data-target="#modalCargarPedido" onclick="CargarPedido(); return false;">CARGAR PEDIDO</a></div>';
        strHtml += "</div>";
      } else {
        strHtml += "</table>";
      }
      ...
      document.getElementById("divResultadoBuscador").innerHTML =
        strHtml + getHtmlTablaResolucionCelular();
...
function getPorcDtoSobrePVP(pIndex) {
  ...
  if (listaProductosBuscados[pIndex].tde_PorcDtoSobrePVP !== null) {
    if (listaProductosBuscados[pIndex].tfr_deshab) {
      ...
      var varTemp_cli_PorcentajeDescuentoDeEspecialidadesMedicinalesDirecto =
        cli_PorcentajeDescuentoDeEspecialidadesMedicinalesDirecto();
      if (
        varTemp_cli_PorcentajeDescuentoDeEspecialidadesMedicinalesDirecto ==
        null
      ) {
        varTemp_cli_PorcentajeDescuentoDeEspecialidadesMedicinalesDirecto = 0;
      }
      var varTemp_tde_PorcARestarDelDtoDeCliente =
        listaProductosBuscados[pIndex].tde_PorcARestarDelDtoDeCliente;
      ...
      var Dto2 =
        parseFloat(
          varTemp_cli_PorcentajeDescuentoDeEspecialidadesMedicinalesDirecto
        ) - parseFloat(varTemp_tde_PorcARestarDelDtoDeCliente);
      var PorcDtoSobrePVP =
        (parseFloat(1) -
          (parseFloat(1) - parseFloat(Dto1) / parseFloat(100)) *
            (parseFloat(1) - parseFloat(Dto2) / parseFloat(100))) *
        parseFloat(100);
      var isRedondeo = true;
      if (
        listaProductosBuscados[pIndex].tfr_pordesadi !== null &&
        listaProductosBuscados[pIndex].tfr_pordesadi > 0
      ) {
        var Dto3 = PorcDtoSobrePVP;
        var Dto4 = parseFloat(listaProductosBuscados[pIndex].tfr_pordesadi);
        PorcDtoSobrePVP =
          (parseFloat(1) -
            (parseFloat(1) - parseFloat(Dto3) / parseFloat(100)) *
              (parseFloat(1) - parseFloat(Dto4) / parseFloat(100))) *
          parseFloat(100);
...
function getPorcDtoSobreOferta(pIndex) {
  ...
  if (!listaProductosBuscados[pIndex].pro_neto) {
    ...
    var varTemp_cli_PorcentajeDescuentoDeEspecialidadesMedicinalesDirecto =
      cli_PorcentajeDescuentoDeEspecialidadesMedicinalesDirecto();
    if (
      varTemp_cli_PorcentajeDescuentoDeEspecialidadesMedicinalesDirecto == null
    ) {
      varTemp_cli_PorcentajeDescuentoDeEspecialidadesMedicinalesDirecto = 0;
    }
    var Dto2 =
    ...
    listaProductosBuscados[pIndex].pro_neto &&
    listaProductosBuscados[pIndex].pro_precio === 0
  ) {
    //2) Si pro_Neto=1  y  pro_ProPrecio=0 -->
    var Dto1 = parseFloat(listaProductosBuscados[pIndex].pro_ofeporcentaje);
    var Dto2 = 0;
    if (listaProductosBuscados[pIndex].pro_codtpopro == "M") {
      var varTemp_PorcentajeDescuentoDeNetosMedicamentos =
        cli_nickname_PorcentajeDescuentoDeNetosMedicamentos();
      if (varTemp_PorcentajeDescuentoDeNetosMedicamentos == null) {
        varTemp_PorcentajeDescuentoDeNetosMedicamentos = 0;
      }
      Dto2 =
        parseFloat(varTemp_PorcentajeDescuentoDeNetosMedicamentos) -
        parseFloat(varTemp_tde_PorcARestarDelDtoDeCliente);
    } else {
      var varTemp_PorcentajeDescuentoDeNetos =
        cli_nickname_PorcentajeDescuentoDeNetos();
      if (varTemp_PorcentajeDescuentoDeNetos == null) {
        varTemp_PorcentajeDescuentoDeNetos = 0;
      }
      Dto2 =
        parseFloat(varTemp_PorcentajeDescuentoDeNetos) -
        parseFloat(varTemp_tde_PorcARestarDelDtoDeCliente);
    }
    var PorcDtoSobrePVP =
      (parseFloat(1) -
    ...
    //3) Si pro_Neto=1 y pro_precio >0 -->
    result =
      (parseFloat(1) -
        parseFloat(listaProductosBuscados[pIndex].PrecioConDescuentoOferta) /
          (parseFloat(listaProductosBuscados[pIndex].pro_precio) /
            parseFloat(1.21))) *
      parseFloat(100);
  }
  return result;
}
function AgregarAlHistorialProductoCarrito_SubirPedido(
  pIndexProducto,
  pIndexSucursal,
  pCantidadProducto,
  pIsSumarCantidad
  ...
  for (
    var iSucursal = 0;
    iSucursal <
    listaProductosBuscados[pIndexProducto].listaSucursalStocks.length;
    iSucursal++
  ) {
    if (
      listaProductosBuscados[pIndexProducto].listaSucursalStocks[iSucursal]
        .stk_codsuc == listaSucursal[pIndexSucursal]
    ) {
      listaProductosBuscados[pIndexProducto].listaSucursalStocks[
        iSucursal
      ].cantidadSucursal = pCantidadProducto;
      ModificarCantidadProductos(
        listaProductosBuscados[pIndexProducto].pro_codigo,
        listaSucursal[pIndexSucursal],
        pCantidadProducto
      );
...
function AgregarAlHistorialProductoCarrito(
  pIndexProducto,
  pIndexSucursal,
  pCantidadProducto,
  pIsSumarCantidad
  ...
  if (isSubirPedido) {
    AgregarAlHistorialProductoCarrito_SubirPedido(
      pIndexProducto,
      pIndexSucursal,
      pCantidadProducto,
      pIsSumarCantidad
...
function CargarOActualizarListaCarrito(
  pIdSucursal,
  pIdProduco,
  pCantidadProducto,
  pIsDesdeBuscador
  ...
  if (
    ...
    if (selectedInput != null) {
      ...
      if (isMoverCursor) {
        ...
        var cantMaxFilaCarrito =
          listaCarritos[indiceCarrito].listaProductos.length;
        ...
        if (!isSalirWhileCarrito) {
          objCarrito = $(
            "#inputCarrito" + indiceCarrito + "_" + indiceCarritoProducto
          );
      ...
      if (isMoverCursor) {
        ...
        var cantMaxFilaTransfer =
          listaTransfer[indiceTransfer].listaDetalle.length;
        switch (keyCode) {
          case 38: //arriba
        ...
        var objTransfer = $(
          "#txtProdTransf" + indiceTransfer + "_" + indiceTransferProducto
        );
...
function CargarCarritos() {
  if (listaCarritos != null) {
    for (var i = 0; i < listaCarritos.length; i++) {
      $("#divContenedorBase_" + listaCarritos[i].codSucursal).html(
        AgregarCarritoHtml(i)
      );
  ...
  if (isNotNullEmpty(args)) {
    ...
    if (isActualizarCarrito) {
      if (
        tempIdSucursal != null &&
        tempIdProduco != null &&
        tempCantidadProducto != null &&
        tempIsDesdeBuscador != null
      ) {
        if (listaCarritos == null) {
          listaCarritos = [];
        }
      ...
      var htmlMensaje =
        ...
        cuerpo_error +
        "</p><ul><li>" +
        obtenerNombreProducto_buscador(
          tempIdProduco,
          tempIdSucursal,
          tempIsDesdeBuscador
        ) +
...
function actualizarCarrito(
  ...
  for (var i = 0; i < listaCarritos.length; i++) {
    if (listaCarritos[i].codSucursal == tempIdSucursal) {
      //
      var sumaTotal = parseFloat(
        String($("#tdTotal" + i).html())
          .replace("$&nbsp;", "")
          .replace(".", "")
          .replace(",", ".")
      );
      var unidadesTotal = parseInt(
        String($("#tdUnidades" + i).html()).replace(textUnidades, "")
      );
        ...
        if (
          ...
          if (listaCarritos[i].listaProductos[iProducto].stk_stock != "N") {
            sumaTotal =
              sumaTotal -
              CalcularPrecioProductosEnCarrito(
                listaCarritos[i].listaProductos[iProducto].PrecioFinal,
                listaCarritos[i].listaProductos[iProducto].cantidad,
                listaCarritos[i].listaProductos[iProducto].pro_ofeunidades,
                listaCarritos[i].listaProductos[iProducto].pro_ofeporcentaje
              );
          }
          unidadesTotal -= parseInt(
            listaCarritos[i].listaProductos[iProducto].cantidad
          );
          ...
          listaCarritos[i].listaProductos[iProducto].cantidad =
            parseInt(tempCantidadProducto);
          isProductoFind = true;
          indiceProducto = iProducto;
          /////
          if (listaProductosBuscados != null) {
            for (
              var iProductoBuscador = 0;
              iProductoBuscador < listaProductosBuscados.length;
              iProductoBuscador++
            ) {
              if (
                listaProductosBuscados[iProductoBuscador].pro_codigo ==
                tempIdProduco
              ) {
                for (
                  var iStock = 0;
                  iStock <
                  listaProductosBuscados[iProductoBuscador].listaSucursalStocks
                    .length;
                  iStock++
                ) {
                  if (
                    listaProductosBuscados[iProductoBuscador]
                      .listaSucursalStocks[iStock].stk_codsuc == tempIdSucursal
                  ) {
                    listaCarritos[i].listaProductos[iProducto].stk_stock =
                      listaProductosBuscados[
                        iProductoBuscador
                      ].listaSucursalStocks[iStock].stk_stock;
                    break;
                  }
                }
                break;
      ...
      if (!isProductoFind) {
        if (listaProductosBuscados != null) {
          for (
            ...
            if (
              ...
              listaProductosBuscados[iProductoBuscador].codProducto =
                tempIdProduco;
              listaProductosBuscados[iProductoBuscador].cantidad =
                parseInt(tempCantidadProducto);
              //
              for (
                var iStock = 0;
                ...
                if (
                  listaProductosBuscados[iProductoBuscador].listaSucursalStocks[
                    iStock
                  ].stk_codsuc == tempIdSucursal
                ) {
                  listaProductosBuscados[iProductoBuscador].stk_stock =
                    listaProductosBuscados[
                      iProductoBuscador
                    ].listaSucursalStocks[iStock].stk_stock;
                  break;
              ...
              //
              listaCarritos[i].listaProductos.push(
                listaProductosBuscados[iProductoBuscador]
              );
              $("#divContenedorBase_" + listaCarritos[i].codSucursal).html(
                AgregarCarritoHtml(i)
        ...
        //Actualizar producto
        if (tempIsDesdeBuscador) {
          $("#inputCarrito" + i + "_" + indiceProducto).val(
            tempCantidadProducto
          );
        }
        // Actualizar Totales
        ...
        if (listaCarritos[i].listaProductos[indiceProducto].stk_stock != "N") {
          var totalTempProducto = CalcularPrecioProductosEnCarrito(
            listaCarritos[i].listaProductos[indiceProducto].PrecioFinal,
            listaCarritos[i].listaProductos[indiceProducto].cantidad,
            listaCarritos[i].listaProductos[indiceProducto].pro_ofeunidades,
            listaCarritos[i].listaProductos[indiceProducto].pro_ofeporcentaje
          );
          strHtmlPrecioProducto =
            "$&nbsp;" +
            FormatoDecimalConDivisorMiles(totalTempProducto.toFixed(2));
        ...
        $("#tdTotal" + i).html(
          "$&nbsp;" + FormatoDecimalConDivisorMiles(sumaTotal.toFixed(2))
        );
        //
  ...
  if (!isCarritoFind) {
    ...
    for (
      ...
      if (
        listaProductosBuscados[iProductoBuscador].pro_codigo == tempIdProduco
      ) {
        ...
        for (
          var iStock = 0;
          iStock <
          listaProductosBuscados[iProductoBuscador].listaSucursalStocks.length;
          iStock++
        ) {
          if (
            listaProductosBuscados[iProductoBuscador].listaSucursalStocks[
    ...
    if (isProducto_isCarritoFind) {
      ...
      if (tempIsDesdeBuscador) {
        ...
        $(
          "#divContenedorBase_" + listaCarritos[indexCarritoAgregar].codSucursal
        ).html(AgregarCarritoHtml(indexCarritoAgregar));
        setScrollFinDeCarrito(indexCarritoAgregar);
        //// Actualizar horario cierre
        indexCarritoHorarioCierre = indexCarritoAgregar;
        ObtenerHorarioCierre(
          listaCarritos[indexCarritoAgregar].codSucursal,
          "OnCallBackObtenerHorarioCierre"
        );
        //indexCarritoHorarioCierre = indexCarritoAgregar;
      ...
      if (listaSucursal[iEncabezadoSucursal] == pIdSucursal) {
        for (var i = 0; i < listaProductosBuscados.length; i++) {
          if (listaProductosBuscados[i].pro_codigo == pIdProducto) {
            var valorCantidad = ObtenerCantidadProductoMasTransfer(
              pIdSucursal,
              listaProductosBuscados[i].pro_codigo,
              listaProductosBuscados[i].pro_nombre
            );
      ...
      if (listaCarritos[i].codSucursal == tempIdSucursal) {
        for (
          ...
          if (
            ...
            $("#inputCarrito" + i + "_" + iProducto).val(
              listaCarritos[i].listaProductos[iProducto].cantidad
            );
            break;
          }
...
function obtenerNombreProducto_buscador(
  pIdProducto,
  pIdSucursal,
  pIsDesdeBuscador
) {
  ...
  if (pIsDesdeBuscador) {
    ...
    if (listaCarritos != null) {
      for (
        var iIndexCarritoSucursal = 0;
        iIndexCarritoSucursal < listaCarritos.length;
        iIndexCarritoSucursal++
      ) {
        if (listaCarritos[iIndexCarritoSucursal].codSucursal == pIdSucursal) {
          for (
            var iProductos = 0;
            iProductos <
            listaCarritos[iIndexCarritoSucursal].listaProductos.length;
            iProductos++
          ) {
            if (
              listaCarritos[iIndexCarritoSucursal].listaProductos[iProductos]
...
function OnCallBackObtenerHorarioCierreCuentaRegresiva(args) {
  if (args != null) {
    ...
    if ((args.length = 2)) new_fechaHorarioCierreSiguiente = args[1];
    else new_fechaHorarioCierreSiguiente = "";
    if (strHtml != "") {
      var hoy = new Date();
      if (strHtml.length == 12) {
        ...
        while (d.getDay() != diaSemanaNro) {
          ...
          if (sumaDia > 7 || d.getDay() == diaSemanaNro) break;
        }
        fechaCuentaRegresiva = d;
      } else {
        strHtml = strHtml.replace(" hs.", "");
        var values = strHtml.split(":");
        fechaCuentaRegresiva = new Date(
          hoy.getFullYear(),
          hoy.getMonth(),
          hoy.getDate(),
          values[0],
...
function countdown() {
  ...
  if (fecha > hoy) {
    ...
    strHtmlReducido +=
      '<div class="suc">Sucursal ' + nombreSucursalDefault + "</div>";
    ...
    strHtml +=
      '<div class="cierre"><i class="fa fa-truck"></i>&nbsp;&nbsp;Cierre de reparto <span>' +
      new_fechaHorarioCierre +
      "</span></div>";
    strHtml +=
      '<div class="faltan">Faltan ' +
      strFaltanDias +
      " <span>" +
      horas +
      ":" +
...
function volverCantidadAnterior_desdeTransfer() {
  if (listaCarritoTransferPorSucursal != null) {
    for (var i = 0; i < listaCarritoTransferPorSucursal.length; i++) {
      volverCantidadAnterior_buscadorTodaSucursal(
    ...
    switch (listaProductosBuscados[pIndex].pro_codtpopro) {
      ...
      case "M":
        if (listaProductosBuscados[pIndex].pro_codtpovta == "VL") {
          ...
          mostrarTipoProducto =
            "Psicotrópico Lista " +
            listaProductosBuscados[pIndex].pro_codtpovta[1];
        } else if (listaProductosBuscados[pIndex].pro_codtpovta[0] == "E") {
          mostrarTipoProducto =
            "Estupefaciente Lista " +
            listaProductosBuscados[pIndex].pro_codtpovta[1];
        }
        break;
      default:
...
function detalleProducto_celular(pIndex) {
  ...
  if (
    !isMostrarImput_AceptaPsicotropicos(
      listaProductosBuscados[pIndex].pro_codtpovta
    )
  ) {
    isMostrarImput = false;
  }
  ...
  if (
    cli_tomaTransfers() &&
    listaProductosBuscados[pIndex].isMostrarTransfersEnClientesPerf
  ) {
    ...
    if (listaProductosBuscados[pIndex].pro_vtasolotransfer) {
      nombre_principal +=
        '<span class="p_trazable">Se vende solo por transfer</span>';
    }
  }
  // Alto Costo
  ...
  strHtml +=
    '<div class="col-lg-12 mpbxs_tit"><button class="btn-modal" onclick="onclickRecuperarTransfer(' +
    pIndex +
    ');">';
  ...
  strHtml +=
    '<a id="vermas_prod" class="btn_a13-plus" href="#" ><i id="plus_vermas_prod" class="fa fa-plus-circle"></i>Ver información del producto</a>';
  ...
  if (listaProductosBuscados[pIndex].pri_nombreArchivo == null) {
    ...
    strHtml +=
      ...
      "productos" +
      "&n=" +
      listaProductosBuscados[pIndex].pri_nombreArchivo +
      "&an=" +
      String(250) +
      "&al=" +
      String(250) +
  ...
  strHtml +=
    '<div class="col-xs-8 no-padding">' + mostrarTipoProducto + "</div>";
  var CadenaFrio = "&nbsp;";
  ...
  if (listaProductosBuscados[pIndex].tfr_descripcion != null) {
    strHtml +=
      '<div class="col-xs-4 no-padding detalleTransferModoCelular">Condición General:</div>';
    strHtml +=
      '<div class="col-xs-8 no-padding detalleTransferModoCelular">' +
      listaProductosBuscados[pIndex].tfr_descripcion +
      "</div>";
  }

  if (listaProductosBuscados[pIndex].tde_descripcion != null) {
    strHtml +=
      '<div class="col-xs-4 no-padding detalleTransferModoCelular">Condición Producto:</div>';
    strHtml +=
      '<div class="col-xs-8 no-padding detalleTransferModoCelular">' +
      listaProductosBuscados[pIndex].tde_descripcion +
      "</div>";
  }
  if (listaProductosBuscados[pIndex].tde_minuni != null) {
    strHtml +=
      '<div class="col-xs-4 no-padding detalleTransferModoCelular">Unidad Mínima:</div>';
    strHtml +=
      '<div class="col-xs-8 no-padding detalleTransferModoCelular">' +
      listaProductosBuscados[pIndex].tde_minuni +
      "</div>";
  }
  if (listaProductosBuscados[pIndex].tde_maxuni != null) {
    strHtml +=
      '<div class="col-xs-4 no-padding detalleTransferModoCelular">Unidad Máxima:</div>';
    strHtml +=
      '<div class="col-xs-8 no-padding detalleTransferModoCelular">' +
      listaProductosBuscados[pIndex].tde_maxuni +
      "</div>";
  }
  if (listaProductosBuscados[pIndex].tde_muluni != null) {
    strHtml +=
      '<div class="col-xs-4 no-padding detalleTransferModoCelular">Múltiplo Unidades:</div>';
    strHtml +=
      '<div class="col-xs-8 no-padding detalleTransferModoCelular">' +
      listaProductosBuscados[pIndex].tde_muluni +
      "</div>";
  }
  if (listaProductosBuscados[pIndex].tde_fijuni != null) {
    strHtml +=
      '<div class="col-xs-4 no-padding detalleTransferModoCelular">Unidades Fijas:</div>';
    strHtml +=
      '<div class="col-xs-8 no-padding detalleTransferModoCelular">' +
      listaProductosBuscados[pIndex].tde_fijuni +
      "</div>";
  }
  if (listaProductosBuscados[pIndex].tde_unidadesbonificadas != null) {
    strHtml +=
      '<div class="col-xs-4 no-padding detalleTransferModoCelular">Unidades Bonificadas:</div>';
    strHtml +=
      '<div class="col-xs-8 no-padding detalleTransferModoCelular">' +
      listaProductosBuscados[pIndex].tde_unidadesbonificadas +
      "</div>";
  }
  //
  ...
  var precioPublico =
    "$&nbsp;" +
    FormatoDecimalConDivisorMiles(
      listaProductosBuscados[pIndex].pro_precio.toFixed(2)
    );
  if (listaProductosBuscados[pIndex].pro_precio === 0) {
    precioPublico = "";
  }
  strHtml +=
    '<div class="col-xs-12 mpbxs_dsc">Público<span class="float-right">' +
    precioPublico +
    "</span></div>";
  var precioHabitual =
    "$&nbsp;" +
    FormatoDecimalConDivisorMiles(
      listaProductosBuscados[pIndex].PrecioFinal.toFixed(2)
    );
  if (listaProductosBuscados[pIndex].pro_nombre.match("^PAÑAL PAMI AD")) {
    precioHabitual = FormatoDecimalConDivisorMiles(
      listaProductosBuscados[pIndex].pro_preciofarmacia.toFixed(2)
    );
  }
  if (listaProductosBuscados[pIndex].pro_AltoCosto) {
    precioHabitual = "";
  ...
  strHtml +=
    '<div class="col-xs-12 mpbxs_dsc">Precio<span class="float-right">' +
    precioHabitual +
    "</span></div>";
  ...
  if (cli_tomaOfertas()) {
    if (
      listaProductosBuscados[pIndex].pro_ofeunidades !== 0 ||
      listaProductosBuscados[pIndex].pro_ofeporcentaje !== 0
    ) {
      ...
      varOfeporcentaje = FormatoDecimalConDivisorMiles(
        getPorcDtoSobreOferta(pIndex).toFixed(2)
      );
      varPrecioConDescuentoOferta =
        "$&nbsp;" +
        FormatoDecimalConDivisorMiles(
          listaProductosBuscados[pIndex].PrecioConDescuentoOferta.toFixed(2)
        );
    }
  }
  ...
  strHtml +=
    '<div class="col-xs-12 mpbxs_dsc">% PVP<span class="float-right">' +
    varOfeporcentaje +
    "</span></div>";
  strHtml +=
    '<div class="col-xs-12 mpbxs_dsc">Min<span class="float-right">' +
    varOfeunidades +
    "</span></div>";
  strHtml +=
    '<div class="col-xs-12 mpbxs_dsc">Precio<span class="float-right">' +
  ...
  if (
    ...
    if (listaProductosBuscados[pIndex].isProductoFacturacionDirecta) {
      if (listaProductosBuscados[pIndex].tde_PorcDtoSobrePVP !== null) {
        varTransferFacturacionDirectaPVP = FormatoDecimalConDivisorMiles(
          getPorcDtoSobrePVP(pIndex).toFixed(2)
        );
      }
      if (
        listaProductosBuscados[pIndex].tde_unidadesbonificadasdescripcion !==
        null
      ) {
        varTransferFacturacionDirectaCondicion =
          listaProductosBuscados[pIndex].tde_unidadesbonificadasdescripcion;
      ...
      if (listaProductosBuscados[pIndex].PrecioFinalTransfer !== null) {
        varTransferFacturacionDirectaPrecio =
          "$&nbsp;" +
          FormatoDecimalConDivisorMiles(
            listaProductosBuscados[pIndex].PrecioFinalTransfer.toFixed(2)
          );
      }
    }
  }
  ...
  strHtml +=
    '<div class="col-xs-12 mpbxs_dsc">% PVP<span class="float-right">' +
    varTransferFacturacionDirectaPVP +
    "</span></div>";
  strHtml +=
    '<div class="col-xs-12 mpbxs_dsc">Cond<span class="float-right">' +
    varTransferFacturacionDirectaCondicion +
    "</span></div>";
  strHtml +=
    '<div class="col-xs-12 mpbxs_dsc">Precio<span class="float-right">' +
  ...
  for (
    ...
    if (isDibujarStock) {
      for (
        var iSucursal = 0;
        iSucursal < listaProductosBuscados[pIndex].listaSucursalStocks.length;
        iSucursal++
      ) {
        if (
          listaProductosBuscados[pIndex].listaSucursalStocks[iSucursal]
            .stk_codsuc === listaSucursal[iEncabezadoSucursal]
        ) {
          var isMostrarImputSucursal = true;
          var isMostrarImputPerfu = true;
          ...
          if (isMostrarImput) {
            is_pedirCC = isMostrarImput_pedirCC(
              listaProductosBuscados[pIndex].pro_codtpopro,
              listaSucursal[iEncabezadoSucursal],
              listaProductosBuscados[pIndex].listaSucursalStocks
            );
          }
          ...
          //}
          if (isMostrarImput) {
            isMostrarImputSucursal = isMostrarImput_FacturaTrazablesProvincia(
              listaSucursal[iEncabezadoSucursal],
              listaProductosBuscados[pIndex].pro_isTrazable
            );
          }
          if (
            isMostrarImput &&
            listaProductosBuscados[pIndex].pro_codtpopro == "P"
          ) {
            isMostrarImputPerfu = MostrarImputPerfu(
              listaProductosBuscados[pIndex].pro_codtpopro,
              listaSucursal[iEncabezadoSucursal],
          ...
          if (
            ...
            if (isSubirPedido) {
              ...
              cantidadDeProductoEnCarrito = ObtenerCantidadProductoMasTransfer(
                listaSucursal[iEncabezadoSucursal],
                listaProductosBuscados[pIndex].pro_codigo,
                listaProductosBuscados[pIndex].pro_nombre
              );
            }
            var typeInput = ' type="number" '; //isMobile()
            if (!isMobile()) typeInput = ' type="text" ';
            strHtml +=
              '<input class="form-shop float-right" id="inputSucCelular' +
              pIndex +
              "_" +
              iEncabezadoSucursal +
              '" ' +
              ...
              '  value="' +
              cantidadDeProductoEnCarrito +
              '"  onkeypress="return onKeypressCantProductos(event)" onfocus="onfocusSucursal(this)" onblur="onblurSucursal_celular(this)"></input>'; //onfocus="onfocusSucursal(this)" onblur="onblurSucursal(this)" onkeypress="return onKeypressCantProductos(event)"
          }
          strHtml +=
            '<div class="' +
            getNameClassStock(
  ...
  strHtml +=
    '<a class="btn_vaciar float-left" href="#" data-dismiss="modal" >CERRAR</a>';
  strHtml += "</div>";
  strHtml += "</div>";
  strHtml += '<div class="clear"></div>';
  strHtml += "</div></div>";
...
function setearValorInputBuscador(pCantidad, pFila, pColumnaSucursal) {
  if (
    $("#inputSuc" + pFila + "_" + pColumnaSucursal).length &&
    $("#inputSuc" + pFila + "_" + pColumnaSucursal).val() != pCantidad
  ) {
    $("#inputSuc" + pFila + "_" + pColumnaSucursal).val(pCantidad);
  }
  if (
    $("#inputSucCelular" + pFila + "_" + pColumnaSucursal).length &&
...
function OnMouseOverProdructoFacturacionDirecta(pIndice) {
  if (
    ...
    timerProductoFacturacionDirecta = setTimeout(function () {
      AnimarPresentacionProductoFacturacionDirecta(pIndice);
    }, 300);
  }
}
function OnMouseOutProdructoFacturacionDirecta() {
  if (
    $("#divMostradorDetalleTransferFacturacionDirecta").css("display") ==
    "block"
  ) {
    $("#divMostradorDetalleTransferFacturacionDirecta").css("display", "none");
  }
...
function OnMouseMoveProdructoFacturacionDirecta(e) {
  ...
  $("#divMostradorDetalleTransferFacturacionDirecta").css(
    "top",
    top +
      (bt || et) +
      20 -
...
function CargarDatosProductosFacturacionDirecta(pIndice) {
  ...
  if (listaProductosBuscados[pIndice].tde_unidadesbonificadas != null) {
    $("#tdUnidadesBonificadas").html(
      listaProductosBuscados[pIndice].tde_unidadesbonificadas
    );
  } else {
    $("#tdUnidadesBonificadas").html("");
  }
  ...
  if (listaProductosBuscados[pIndice].tde_descripcion != null) {
    $("#tdDescripcionUnidadesBonificadas").html(
      listaProductosBuscados[pIndice].tde_descripcion
    );
  } else {
    $("#tdDescripcionUnidadesBonificadas").html("");
  }
  ...
  if (listaProductosBuscados[pIndice].tfr_descripcion != null) {
    $("#tdDescripcionTransfer").html(
      listaProductosBuscados[pIndice].tfr_descripcion
    );
  } else {
    $("#tdDescripcionTransfer").html("");
  }
...
function focusPrimerProductoHabilitado() {
  ...
  if (listaProductosBuscados != null && listaProductosBuscados.length > 0) {
    for (var iSucursal = 0; iSucursal < listaSucursal.length; iSucursal++) {
      for (
        var iProducto = 0;
        iProducto < listaProductosBuscados.length;
        iProducto++
      ) {
