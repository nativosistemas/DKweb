...
var objMensajeEstaSeguroDeseaEliminarRegistro =
  "¿Está seguro de que desea eliminar el registro?";
var objMensajeEstaSeguroDeseaCambiarRegistro =
  "¿Está seguro de que desea cambiar el estado el registro?";
var objMensajeNoSePudoDescargarArchivoInténteloNuevamente =
...
$(document).ready(function () {
  ...
  if (listaMensajeImportanteMostrar == null) {
    listaMensajeImportanteMostrar = eval(
      "(" + $("#hiddenListaMensajeImportante").val() + ")"
    );
...
function MostrarMensajeImportante() {
  if (listaMensajeImportanteMostrar != null) {
    if (listaMensajeImportanteMostrar.length > 0) {
      var indexImportante =
        listaMensajeImportanteMostrar.length - longMensajeImportanteMostrar;
      longMensajeImportanteMostrar = longMensajeImportanteMostrar - 1;
      mensaje_EnInicio(
        listaMensajeImportanteMostrar[indexImportante].tme_asunto,
        listaMensajeImportanteMostrar[indexImportante].tme_mensaje
      );
...
function OnCallBackObtenerSaldoFinalADiciembrePorCliente(args) {
  ...
  if (isNotNullEmpty(args)) {
    strSaldo =
      "$&nbsp;" +
      FormatoDecimalConDivisorMiles(Number.parseFloat(args).toFixed(2));
  }
...
function isMostrarImput_FacturaTrazablesProvincia(
  pSucursal,
  pIsProductoTrazable
) {
  if (listaSucursales != null) {
      ...
      for (var i = 0; i < listaSucursales.length; i++) {
        if (pSucursal == listaSucursales[i].sde_sucursal) {
          if (!listaSucursales[i].suc_facturaTrazables) {
            ...
            !listaSucursales[i].suc_facturaTrazablesEnOtrasProvincias &&
...
function isMostrarImput_pedirCC(
  pPro_codtpopro,
  pSucursalEvaluar,
  pListaSucursalStocks
) {
  ...
  if (
    ...
    ((pPro_codtpopro == "P" && sucursalInfo.suc_pedirCC_tomaSoloPerfumeria) || //TIPOPRODUCTO_Perfumeria
    ...
    for (
      var iSucursal = 0;
      iSucursal < pListaSucursalStocks.length;
      iSucursal++
    ) {
...
function MostrarImputPerfu(pPro_codtpopro, pSucursalEvaluar) {
  if (listaSucursales != null) {
    for (var iSucursal = 0; iSucursal < listaSucursales.length; iSucursal++) {
      if (
        listaSucursales[iSucursal].sde_sucursal === pSucursalEvaluar &&
        listaSucursales[iSucursal].suc_trabajaPerfumeria
      ) {
        return true;
        break;
      }
...
function getCantidad_SubirArchivo_pedirCC(
  pPro_codtpopro,
  pSucursalEvaluar,
  pListaSucursalStocks
) {
  ...
  if (sucursalInfo != null) {
    ...
    if (
      pSucursalEvaluar == sucReferencia &&
      !sucursalInfo.suc_pedirCC_ok &&
      ((pPro_codtpopro == "P" && sucursalInfo.suc_pedirCC_tomaSoloPerfumeria) || //TIPOPRODUCTO_Perfumeria
        !sucursalInfo.suc_pedirCC_tomaSoloPerfumeria)
    ) {
      //TIPOPRODUCTO_Perfumeria
      for (
        var iSucursal = 0;
        iSucursal < pListaSucursalStocks.length;
        iSucursal++
      ) {
        if (pListaSucursalStocks[iSucursal].stk_codsuc === "CC") {
          // Casa central
          if (
            isNotNullEmpty(pListaSucursalStocks[iSucursal].cantidadSucursal)
          ) {
